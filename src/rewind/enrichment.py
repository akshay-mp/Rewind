...
3. :func:`sample_vram` — one-shot VRAM / unified-memory sample on Apple
   ...
   periodically during a ``rewind ui`` session and stamp samples against
...
* **No hard dependencies on transformers, macmon, or nvidia-smi.** Each
  ...
  The hard guarantee: ``import rewind.enrichment`` never fails, regardless
...
* **No new SQLite columns.** Enrichment output is written into
  ``Span.raw_attributes`` under ``rewind.local.*`` keys — the existing
...
def quant_from_span(span: Span) -> QuantInfo:
    ...
    The diff auto-flag uses this: if a span already has ``rewind.local.quant``
...
def sample_vram() -> VramSample:
    ...
    raises — this is sampled from the ``rewind ui`` background thread and
...
def enrich_span(
    ...
    * ``sample_gpu`` — :func:`sample_vram` → ``VRAM_MIB_ATTR`` +
    ...
    ``rewind render-template <span>`` inspection command instead.
    """
