...
This module is **pure logic**. It depends only on :class:`rewind.storage.TraceStore`
and :mod:`rewind.models`; it does **not** import OpenAI, LangGraph, or any
network client. The interceptors in :mod:`rewind.openai_intercept`,
:mod:`rewind.tool_intercept`, and :mod:`rewind.adapters.langgraph` layer on
...
_active_session: ContextVar[ReplaySession | None] = ContextVar(
    "rewind_active_session", default=None
)
...
class CallSignature:
    ...
    :func:`rewind.models.hash_payload`). ``tools_hash`` is the hash of the
    ...
    * ``approval`` — optional :class:`~rewind.stepping.ApprovalChannel`.
    ...
    def fork(
        ...
        *,
        ...
        :meth:`~rewind.storage.TraceStore.get_spans`.
    ...
    def advance_cursor_to(self, index: int) -> None:
        ...
        Used by :mod:`rewind.tool_intercept` when it matches a TOOL span
        ...
        if index < self._cursor:
            raise ReplayError(
                ...
                "tool spans cannot rewind an already-consumed cursor"
            )
    ...
    def trace_summary(self) -> Trace:
        """Return a :class:`~rewind.models.Trace` view of this branch's spans.
