...
Track 3B.3 of plan §6 Phase 3. During a frozen :func:`rewind.replay`,
...
* ``span.kind`` is :attr:`~rewind.enums.SpanKind.TOOL` or :data:`.MCP`
...
* FROZEN mode: raise :class:`~rewind.replay.ReplayError` — fail closed.
* BRANCH / FULL_RERUN modes: call through and capture the new TOOL span
  under the active branch via :meth:`~rewind.replay.ReplaySession.record_new`.

Reentrancy: same contract as :mod:`rewind.openai_intercept` — each replay
...
def tool(
    ...
    *,
    ...
    """Wrap a Python function as a Rewind-replay-aware tool.
    ...
    * Active session + mismatch in BRANCH / FULL_RERUN → call live and
    ...
    def _decorator(func: F) -> F:
        ...
        wrapper.__rewind_tool_name__ = tool_name  # type: ignore[attr-defined]
        wrapper.__rewind_tool_kind__ = tool_kind  # type: ignore[attr-defined]
...
def _find_tool_span(
    ...
    *,
    ...
    for idx in range(cursor, len(cache)):
        ...
        from rewind.replay import RecordedResponse  # pylint: disable=import-outside-toplevel
...
def _materialise_tool_output(payload: dict[str, Any]) -> Any:
    ...
    We trust the recording — Rewind is a debugging tool, not a sanitiser.
    """
...
# ----------------------------------------------------------------------
# Session plumbing — forwarded to rewind.replay (kept here for testability)
# ----------------------------------------------------------------------
