...
takes :class:`~rewind.models.Span` lists in and produces render-friendly
...
def _base_quant_pairs(spans: list[Span]) -> set[tuple[str, str | None]]:
    ...
    from rewind.enrichment import quant_from_span
...
class BranchNode:
    ...
    # The seven fixed fields mirror ``rewind.models.Branch`` verbatim;
    ...
    # the field list against ``rewind.timeline.BranchNodeView`` — the
