...
consulting the active :class:`~rewind.replay.ReplaySession`:
...
imported inside the factory so ``rewind --version`` stays fast without
...
def replay_model(
    ...
    *,
    ...
    class _ReplayModel(Model):  # type: ignore[misc]
        ...
        @property
        def system(self) -> str:
            wrapped_system = getattr(self._rewind_wrapped, "system", "rewind")
            return f"rewind-replay({wrapped_system})"

        @property
        def model_name(self) -> Any:
            return getattr(self._rewind_wrapped, "model_name", "rewind-replay")

        @property
        def system_api(self) -> str:
            return getattr(self._rewind_wrapped, "system_api", "rewind")
        ...
        async def request(
            ...
            *,
            ...
            if session is None:
                return await self._rewind_wrapped.request(
                    messages,
            ...
            if recorded is None:
                ...
                result = await self._rewind_wrapped.request(
                    messages,
                    model_settings=model_settings,
                    model_request_parameters=model_request_parameters,
                )
