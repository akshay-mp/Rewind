...
during a :func:`rewind.replay` context, its ``_generate`` / ``_agenerate``
consult the active :class:`~rewind.replay.ReplaySession`:
...
* Active + divergence in FROZEN → raise :class:`~rewind.replay.ReplayError`.
* No active session → delegate to the wrapped model verbatim.

The factory pattern (rather than a top-level class) means **Rewind works
on machines without ``langchain_core`` installed** — the import only fires
when the adapter is actually requested. This keeps ``rewind --version``
...
def replay_chat_model(
    ...
    *,
    ...
    BaseChatModel
        ...
        through Rewind. The original model is preserved as ``._rewind_wrapped``.
    """
    ...
    try:
        ...
        raise AdapterError(
            "rewind.adapters.langgraph requires `langchain-core`; install it or use "
            "the generic OpenAI monkey-patch (rewind.openai_intercept.patch)."
        ) from exc
    ...
    class _ReplayChatModel(BaseChatModel):  # type: ignore[misc]
        ...
        _rewind_wrapped: BaseChatModel = wrapped
        ...
        def _llm_type(self) -> str:
            return f"rewind-replay({self._rewind_wrapped._llm_type})"
        ...
        def _generate(
            ...
            **kwargs: Any,
            ...
            if session is None:
                return self._rewind_wrapped._generate(
        ...
        async def _agenerate(
            ...
            **kwargs: Any,
            ...
            session = self._active_session()
            if session is None:
                return await self._rewind_wrapped._agenerate(
                    messages, stop=stop, run_manager=run_manager, **kwargs
                )
        ...
        def _dispatch_sync(
            ...
            **kwargs: Any,
            ...
            if recorded is None:
                ...
                result = self._rewind_wrapped._generate(
                    messages, stop=stop, run_manager=run_manager, **kwargs
                )
        ...
        async def _dispatch_async(
            ...
            **kwargs: Any,
            ...
            if recorded is None:
                self._assert_not_frozen(session)
                result = await self._rewind_wrapped._agenerate(
                    messages, stop=stop, run_manager=run_manager, **kwargs
                )
                self._capture_live_span(session, messages, result, signature.model)
                return result
        ...
        def _signature(self, messages: list[BaseMessage], **kwargs: Any) -> Any:
            ...
            return extract_signature(
                model=str(kwargs.get("model") or self._rewind_wrapped._llm_type),
        ...
        def _capture_live_span(
            ...
            span = Span(
                ...
                span_id=token_hex(8),
                parent_span_id=None,
                name=f"langchain.{self._rewind_wrapped._llm_type}",
                kind=SpanKind.LLM,
