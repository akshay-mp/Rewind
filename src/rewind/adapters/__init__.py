"""Rewind framework adapters — bridge recorded spans to live agent loops.
...
* :mod:`rewind.adapters.langgraph` — :func:`replay_chat_model` wraps
  a ``langchain_core.BaseChatModel``.
* :mod:`rewind.adapters.adk` — :func:`replay_llm` wraps a
  ``google.adk.models.llms.BaseLlm``.
* :mod:`rewind.adapters.pydantic_ai` — :func:`replay_model` wraps a
  ``pydantic_ai.models.Model``.
* :mod:`rewind.adapters.crewai` — :func:`replay_llm` wraps a
  ``crewai.llms.base_llm.BaseLLM``.
* :mod:`rewind.adapters.smolagents` — :func:`replay_model` wraps a
  ``smolagents.models.Model``.

Imports remain lazy: the user's project only pays for the framework
they're actually replaying, and ``rewind --version`` stays fast when no
