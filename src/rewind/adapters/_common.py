...
1. Lazy-import the framework inside the factory function (so
   ``rewind --version`` stays fast when frameworks aren't installed).
2. Subclass or wrap the framework's chat-model surface (``BaseLlm``,
   ``BaseChatModel``, ``Model``, ``HfApiModel``…).
3. For each inference call:
   - Look up the active :class:`~rewind.replay.ReplaySession` via
     :func:`~rewind.replay.active_session`.
   - Build a :class:`~rewind.replay.CallSignature` (re-using
     :func:`~rewind.openai_intercept.extract_signature` when the framework
     accepts ``model=…, messages=[…], tools=[…]`` — ask all five do today).
   - Call ``session.respond_or_forward(signature)``:
     * returns a :class:`~rewind.replay.RecordedResponse` → materialise
...
def build_live_span(
    ...
    *,
    ...
    model_name:
        The model the live call hit (``self._rewind_wrapped._llm_type`` etc).
    messages:
    ...
    if kind is SpanKind.TOOL:
        raw: dict[str, Any] = {
            "tool.name": tool_name or "rewind-replay",
            "tool.output": content or "",
            **(raw_extras or {}),
        }
        name = f"tool.{tool_name or 'rewind'}"
    else:
        raw = {
        ...
        name = f"rewind.adapter.{model_name or 'unknown'}"
