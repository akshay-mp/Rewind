...
while consulting the active :class:`~rewind.replay.ReplaySession`:
...
def replay_model(
    ...
    *,
    ...
    -------
    Model
        A subclass instance whose ``__call__`` routes through Rewind. The
        wrapped model is preserved as ``._rewind_wrapped``.
    """
    try:
    ...
    class _ReplayModel(Model):  # type: ignore[misc]
        ...
        def _model_id(self) -> str:
            return str(
                getattr(self._rewind_wrapped, "model_id", "smolagents-replay")
