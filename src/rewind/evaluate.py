...
2. **The suite runner** (:func:`evaluate`): the asynchronous orchestrator
   ...
   :class:`~rewind.replay.ReplaySession`, runs the requested evaluators,
...
Why pure + async-orchestrator in one file?
    ...
    one file. The HTTP + CLI surfaces live in :mod:`rewind.timeline_api`
    and :mod:`rewind.cli`, respectively.

Concurrency guarantees
----------------------
Parallel safety leans on three Phase 3/4 invariants:

* :class:`~rewind.replay.ReplaySession` is per-branch isolated (its cursor
...
* :class:`~rewind.storage.TraceStore` writes serialise on the SQLite WAL
...
* It does not write to ``~/.rewind`` directly; persistence of the
...
class EvalScenario:
    ...
    :class:`~rewind.replay.ReplaySession`, drives the candidate branch,
    and scores it.
...
class SuiteValidationError(ValueError):
    ...
    (those become :class:`~rewind.replay.ReplayError` -> SKIP/ERROR
    verdicts on individual scenarios).
    """


class ReplaySessionFactory(Protocol):
    """Hook the test layer injects to mock :func:`rewind.replay.replay`.

    Production calls go through :func:`_default_replay_session_factory`
    which opens a real :class:`~rewind.replay.ReplaySession`. Tests can
...
def _candidate_mode_to_replay_mode(mode: CandidateMode) -> ReplayMode:
    """Translate :class:`CandidateMode` → :class:`~rewind.enums.ReplayMode`.
...
async def evaluate(
    ...
    *,
    ...
    Args:
        ...
        store: The :class:`~rewind.storage.TraceStore` to fork from.
    ...
    :class:`~rewind.replay.ReplaySession`; concurrent sessions never
    share cursor state.
    """
...
def _utcnow_iso() -> str:
    """ISO-8601 timestamp helper. Mirrors :mod:`rewind.models._utcnow_iso`.
