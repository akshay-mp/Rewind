...
:class:`func:`rewind.replay`` context each intercepted call:

* pauses at the stepping gate (:func:`rewind.stepping.gate_async` /
...
if TYPE_CHECKING:
    ...
    from rewind.stepping import Decision, Step
...
def patch() -> Iterator[None]:
    ...
    try:
        ...
        raise InterceptError(
            "rewind workbench LangGraph activation requires `langchain-core`; "
            "install it with `pip install rewind-debugger[langgraph]`."
        ) from exc
    ...
    def patched_tool_invoke(self: Any, input: Any, config: Any = None, **kwargs: Any) -> Any:
        session = active_session()
        if session is None or _is_rewind_tool(self):
    ...
    for patched in (patched_invoke, patched_ainvoke, patched_tool_invoke, patched_tool_ainvoke):
        patched.__rewind_patched__ = True  # type: ignore[attr-defined]
...
def _is_replay_wrapper(model: Any) -> bool:
    ...
    return getattr(model, "_rewind_wrapped", None) is not None
...
async def _complete_llm_step(
    ...
    # pylint: disable=import-outside-toplevel
    from rewind.stepping import complete_step
    # pylint: enable=import-outside-toplevel
...
# Tool path
# ----------------------------------------------------------------------
def _is_rewind_tool(tool: Any) -> bool:
    """True when the tool's function already dispatches via ``@rewind.tool``."""
    return getattr(getattr(tool, "func", None), "__rewind_tool_name__", None) is not None
...
def _json_safe(value: Any) -> Any:
    ...
    from rewind.openai_intercept import _to_jsonable
    # pylint: enable=import-outside-toplevel
...
def _tool_step(
    ...
    """Build the TOOL step payload (shape shared with ``@rewind.tool``)."""
    # pylint: disable=import-outside-toplevel
...
def _no_call_result(decision: Decision, tool_name: str) -> Any:
    ...
    if decision.kind is DecisionKind.SKIP:
        return {"rewind": "tool skipped", "tool": tool_name}
...
def _tool_replay_lookup(
    ...
    from rewind.tool_intercept import _find_tool_span, _tool_args_hash
    # pylint: enable=import-outside-toplevel
...
def _decode_tool_output(stored: Any) -> Any:
    ...
    if not isinstance(stored, dict) or "__rewind_command__" not in stored:
        return stored
    envelope = stored["__rewind_command__"]
    # pylint: disable=import-outside-toplevel
    try:
...
def _capture_live_tool(
    ...
    # pylint: disable=import-outside-toplevel
    from rewind.enums import SpanKind
    from rewind.tool_intercept import _capture_live_tool_span, _tool_args_hash
    # pylint: enable=import-outside-toplevel
