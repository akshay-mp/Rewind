...
2. Any uncommitted working-tree/index changes (the **delta**) via
   ``git stash``. These are popped back on rewind.

On :meth:`on_rewind` we run, in order: ``git reset --hard <anchor>``
(rewind committed state), ``git clean -fd`` (drop untracked files the
...
is treated as a trusted binary; users who pin Rewind behind a sandbox
...
from rewind.rollback.base import RollbackError

#: Tag prefix Rewind stamps onto every stash entry. Lets ``git stash list``
#: show Rewind-managed stashes distinctly from user stashes; also scopes
#: ``git stash drop`` to entries we own.
_REWIND_STASH_PREFIX = "rewind-branch-"
...
class GitRollbackHandler:
    ...
    The handler has no Python-level state across branch/rewind pairs: the
    ...
    def _git_or_none(self, *args: str) -> str | None:
        ...
        (e.g. verifying an anchor ref during :meth:`on_rewind` — the ref
    ...
    def on_branch(self, branch_id: UUID) -> None:
        ...
        rewind: the current HEAD commit (the *anchor*) and any uncommitted
        working-tree changes (the *delta*). The delta is stashed under a
        ``rewind-branch-<branch_id.hex>`` message tag so it can be located
        ...
        # only state on rewind that's strictly required — even if the
        ...
        # reading a small ref that ``on_rewind`` looks up. Using a real
    ...
    def on_rewind(self, branch_id: UUID) -> None:
        ...
        1. Look up the recorded anchor via ``refs/rewind/<tag>``.
        ...
        4. If a stash with the branch tag exists, ``git stash pop`` it to
           restore pre-branch uncommitted changes.
        5. Drop the ``refs/rewind/<tag>`` ref (cleanup; the branch is gone).
        ...
        anchor = self._git_or_none("rev-parse", "--verify", "--quiet", f"refs/rewind/{tag}")
        if not anchor:
            return

        # Step 2: rewind commits. ``--hard`` also resets the index, so any
        ...
        try:
            ...
            raise RollbackError(
                f"on_rewind({tag}): git reset --hard {anchor[:8]} failed — "
        ...
        try:
            ...
            raise RollbackError(
                f"on_rewind({tag}): git clean -fd failed after reset — "
        ...
        if stash_ref is not None:
            try:
                ...
                raise RollbackError(
                    f"on_rewind({tag}): stash pop failed — the reset+clean "
        ...
        # even an interrupted rewind leaves a recoverable reflog trail.
        try:
            self._git("update-ref", "-d", f"refs/rewind/{tag}")
    ...
    def _find_stash(self, tag: str) -> str | None:
        ...
        for line in listing.splitlines():
            if needle in line:
                # Lines look like ``stash@{0}: On main: on_branch(rewind-...)``
                colon = line.find(":")
                if colon > 0:
                    return line[:colon]
        return None
