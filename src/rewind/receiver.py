...
- ``POST /v1/traces`` — accepts OTLP/HTTP in either protobuf
  ...
  Rewind ``Span``s, and persisted via :class:`rewind.storage.TraceStore`.
...
<html lang="en">
  <head><meta charset="utf-8"><title>Rewind UI not built</title></head>
  <body>
    <h1>Rewind UI not built</h1>
...
def create_app(store: TraceStore, registry: Rewind | None = None) -> FastAPI:
    ...
    Mounts the read-only timeline API (:func:`rewind.timeline.mount_timeline`)
    and serves the static UI artifact if it has been built.
    """
    app = FastAPI(
        title="Rewind OTLP/HTTP receiver",
        version=__version__,
        docs_url=None,
        redoc_url=None,
...
def _register_routes(app: FastAPI) -> None:
    ...
    async def ingest_traces(request: Request) -> Response:
        ...
        return Response(
            ...
            headers={"x-rewind-spans-accepted": str(count)},
        )
