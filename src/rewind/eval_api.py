...
bloating :mod:`rewind.timeline`. The harness has its own lifecycle
...
Concurrency
-----------
``POST /api/v1/evals`` runs the suite through :func:`rewind.evaluate.evaluate`
...
def mount_eval(app: FastAPI) -> None:
    ...
    Mirrors :func:`rewind.timeline.mount_timeline` — same app.state.store
    accessor, same exception conventions. Should be called after
    :func:`~rewind.timeline.mount_timeline` so the UI can deep-link into
