...
the :class:`~rewind.replay.ReplaySession` contract.
...
* :class:`ApprovalChannel` — the protocol a dispatcher awaits on. The agent
  ...
  sync ``@rewind.tool()`` path; the sync side blocks on a
...
* **Per-task isolation.** The channel lives on the
  :class:`~rewind.replay.ReplaySession`, which is stored in a
...
class StepKind(StrEnum):
    ...
    Mirrors :class:`~rewind.enums.SpanKind` minus ``UNKNOWN`` — stepping only
    applies to LLM and tool calls (you don't pause at a framework
    orchestration node, because Rewind doesn't intercept those).
    """
...
class Step:
    ...
    * For a tool step: ``{"name": str, "args": [...], "kwargs": {...}}``.
    ...
    can emit its native message format without a Rewind-side projection.
    """
...
class SteppingStopped(RuntimeError):
    ...
    Distinct from :class:`~rewind.replay.ReplayError` (which signals a
...
class ThreadBridgeChannel:
    """Sync→async bridge for the ``@rewind.tool()`` path.
...
def gate_sync(session: ReplaySession, step: Step) -> Decision | None:
    """Sync dual of :func:`gate_async`, for the ``@rewind.tool()`` path.
