...
the active :class:`~timetravel.replay.ReplaySession`:
...
def replay_llm(
    ...
    *,
    ...
    BaseLLM
        ...
        TimeTravel. The wrapped LLM is preserved as ``._timetravel_wrapped``.
    """
    try:
    ...
    class _ReplayLLM(BaseLLM):  # type: ignore[misc]
        ...
        @property
        def model(self) -> str:
            return f"timetravel-replay({getattr(self._timetravel_wrapped, 'model', 'crewai')})"

        @property
        def _llm_type(self) -> str:
            return getattr(self._timetravel_wrapped, "_llm_type", "crewai")
        ...
        async def call_async(self, messages: list[Any], **kwargs: Any) -> Any:
            session = self._active_session()
            async_call = getattr(self._timetravel_wrapped, "call_async", None)
            if session is None:
        ...
        def supports_function_calling(self) -> bool:
            return bool(getattr(self._timetravel_wrapped, "supports_function_calling", lambda:
                False)())
        ...
        def _model_name(self, **kwargs: Any) -> str:
            ...
            return str(
                getattr(self._timetravel_wrapped, "model", "crewai-replay")
                or "crewai-replay"
            )
