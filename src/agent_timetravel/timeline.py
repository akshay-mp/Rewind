...
- ``GET /api/v1/spans/{timetravel_id}`` — single span with raw attributes expanded.
...
def _register_routes(app: FastAPI) -> None:  # pylint: disable=too-many-statements
    ...
    @app.get("/api/v1/spans/{timetravel_id}", tags=["timeline"])
    def get_span(request: Request, timetravel_id: UUID) -> SpanView:
        """Return a single span by ``timetravel_id``.
        ...
        """
        store: TraceStore = request.app.state.store
        span = store.get_span(timetravel_id)
        if span is None:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
                detail=f"span {timetravel_id} not found",
            )
    ...
    @app.get(
        "/api/v1/spans/{timetravel_id}/message-diff",
        tags=["timeline", "diff"],
    )
    def diff_messages(
        request: Request,
