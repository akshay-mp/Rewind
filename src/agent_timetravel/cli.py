...
if TYPE_CHECKING:
    ...
    from agent_timetravel.evaluate import EvalSuiteResult
...
_DEFAULT_PORT = 4318
#: Default database location: ``~/.timetravel/timetravel.db``. Phase 8 packaging
...
_DEFAULT_DB = Path.home() / ".agent-timetravel" / "timetravel.db"
...
@click.group(cls=_TimeTravelGroup)
@click.version_option(version=__version__, prog_name="timetravel")
def cli() -> None:
    """TimeTravel — time-travel debugging for AI agents."""


@cli.command()
def version() -> None:
    """Print the installed TimeTravel version."""
...
def dev(application: str, host: str, port: int, db_path: Path, no_open: bool) -> None:
    ...
    if ":" not in application:
        raise click.ClickException(
            ...
            "a timetravel.TimeTravel instance, a LangGraph graph, or a callable"
        )
    ...
    try:
        ...
        raise click.ClickException(
            ...
            "export your TimeTravel registry, LangGraph graph, or callable "
            "under that name"
        ) from exc
    # pylint: disable=import-outside-toplevel
    from agent_timetravel import TimeTravel
    from agent_timetravel.graph_app import GraphAppError, registry_from_object
    # pylint: enable=import-outside-toplevel

    if isinstance(application_object, TimeTravel):
...
def _open_when_ready(url: str, *, timeout: float = 15.0) -> None:
    ...
    threading.Thread(target=_open, daemon=True, name="timetravel-open-ui").start()
...
def replay(
    ...
    try:
        ...
        click.echo(f"timetravel: replay error: {exc}", err=True)
...
def eval_cmd(
    ...
    from agent_timetravel.eval_api import parse_suite_from_yaml
    from agent_timetravel.evaluate import EvalSuite, SuiteValidationError, evaluate
    from agent_timetravel.storage import TraceStore
...
def main() -> None:
    """Entrypoint referenced by ``[project.scripts] timetravel``."""
    cli()
...
def checkpoint_list(
    ...
    if branch_id is not None:
        try:
            ...
            click.echo(f"timetravel: --branch must be a UUID, got {branch_id!r}", err=True)
            raise click.exceptions.Exit(2) from exc
    ...
    if not branches:
        click.echo(f"timetravel: no branches found for trace {trace_id}", err=True)
        raise click.exceptions.Exit(1)
...
def checkpoint_restore(
    ...
    if cp is None:
        click.echo(
            f"timetravel: no checkpoint named {name!r} on branch {branch_id}",
            err=True,
        )
        raise click.exceptions.Exit(1)
...
@click.option(
    ...
    "stamp the result onto span.raw_attributes['agent_timetravel.local.quant'].",
)
@click.option(
    "--vram/--no-vram",
    "sample_vram_flag",
...
def enrich(
    ...
    from agent_timetravel.enrichment import enrich_span
    from agent_timetravel.storage import TraceStore
    # pylint: enable=import-outside-toplevel
    ...
    if bid is None:
        ...
        if trace is None:
            click.echo(f"timetravel: no such trace {trace_id!r}", err=True)
            raise click.exceptions.Exit(1)
        spans = trace.spans
    else:
        ...
        if not spans:
            click.echo(
                f"timetravel: no spans found for trace {trace_id!r} branch {branch_id!r}",
                err=True,
            )
            raise click.exceptions.Exit(1)
...
def render_template(
    ...
    from agent_timetravel.enrichment import render_chat_template
    from agent_timetravel.storage import TraceStore
    # pylint: enable=import-outside-toplevel
    ...
    if span_index < 0 or span_index >= len(spans):
        click.echo(
            f"timetravel: span_index {span_index} out of range "
    ...
    if not isinstance(messages, list):
        click.echo(
            "timetravel: span has no messages to render "
...
def regression_create(
    ...
    if store.get_trace(seed_trace_id) is None:
        click.echo(f"timetravel: seed trace {seed_trace_id!r} not found", err=True)
        raise click.exceptions.Exit(1)
...
def regression_run(
    ...
    ``timetravel regression create``). Pass ``all`` to run every case.
    ...
    if not ids:
        click.echo("timetravel: no regression cases to run", err=True)
        raise click.exceptions.Exit(2)
...
def export(
    ...
    \b
    Examples:
      timetravel export <trace_id> -o trace.json
      timetravel export <trace_id> --redact-field gen_ai.response
      timetravel export <trace_id> --redact-pattern '\\b\\d{3}-\\d{2}-\\d{4}\\b'
      timetravel export <trace_id> --preview --redact-pattern '[A-Z]{4}'
    """
    # pylint: disable=import-outside-toplevel
    import json
    ...
    trace = store.get_trace(trace_id)
    if trace is None:
        click.echo(f"timetravel: trace {trace_id!r} not found", err=True)
        raise click.exceptions.Exit(1)
