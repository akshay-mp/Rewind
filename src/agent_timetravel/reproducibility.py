...
def capture_manifest(*, timetravel_version: str = "") -> RunManifest:
    """Capture the current environment as a :class:`RunManifest`.

    ``timetravel_version`` defaults to :data:`timetravel.__version__` when empty.
    Package versions are read from :mod:`importlib.metadata` for the timetravel
    install + its direct dependencies; missing packages are skipped silently.
    """
    if not timetravel_version:
        # pylint: disable=import-outside-toplevel
        try:
            from agent_timetravel import __version__ as _v
        except ImportError:
            _v = "unknown"
        timetravel_version = _v

    packages = _collect_package_versions()
    manifest = RunManifest(
        timetravel_version=timetravel_version,
...
def _collect_package_versions() -> dict[str, str]:
    """Read versions for timetravel + its known dependencies from importlib.metadata."""
    # pylint: disable=import-outside-toplevel
    try:
...
def _with_content_hash(manifest: RunManifest) -> RunManifest:
    ...
    payload = json.dumps(
        {
            "timetravel_version": manifest.timetravel_version,
