...
* ``on_timetravel`` is called when a branch is being abandoned — it should
  restore the snapshot taken at ``on_branch`` time and clean up the tag.
* Branch ids are UUIDs (the :class:`~timetravel.replay.ReplaySession.branch_id`).
...
class RollbackHandler(Protocol):
    ...
    a refresh, not an error; calling ``on_timetravel`` on an unknown id is a
    no-op, not an error).
    """
    ...
    def on_branch(self, branch_id: UUID) -> None:
        ...
        Called by the active :class:`~timetravel.replay.ReplaySession` before
