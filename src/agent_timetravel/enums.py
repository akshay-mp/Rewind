"""Core domain enums for TimeTravel.

Mirrors the OpenTelemetry GenAI semantic conventions span `gen_ai.system`
values plus TimeTravel's internal classification of span kinds. We keep an eye on
...
class SpanKind(StrEnum):
    ...
    ``SpanKind`` enum (CLIENT/SERVER/...). TimeTravel cares about the *agent
    ...
    UNKNOWN = "agent_timetravel.unknown"
...
class ReplayMode(StrEnum):
    ...
    ``INTERACTIVE``
        ...
        attached to the :class:`~timetravel.replay.ReplaySession`; without one it
...
class SpanStatus(StrEnum):
    ...
    optional ``status.message``. We collapse into three TimeTravel-facing states.
    """
...
class EvaluatorKind(StrEnum):
    ...
    ("Eval harness overlaps with Agents_Arena"). TimeTravel implements each
