...
def registry_from_object(obj: Any, name: str = "graph") -> TimeTravel:
    """Build a one-agent :class:`TimeTravel` registry around ``obj``.
    ...
    raise GraphAppError(
        f"{name!r} is {type(obj).__name__}; expected a timetravel.TimeTravel instance, "
        "a LangGraph graph / langchain runnable, or a callable"
    )


def _registry_from_graph(graph: Any, *, name: str) -> TimeTravel:
    registry = TimeTravel(title=name)
    ...
    registry.agent(
        ...
        description=(
            ...
            "TimeTravel builds the graph input."
        ),
    )(run)
    return registry


def _registry_from_callable(func: Any, *, name: str) -> TimeTravel:
    if any(
