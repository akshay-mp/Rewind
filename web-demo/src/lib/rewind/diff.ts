/**
 * Rewind diff engine — mirrors src/rewind/diff.py from akshay-mp/Rewind.
 *
 * Two layers:
 ...
 *      divergence index" — the headline metric Rewind reports).
 *
...
export function diffBranches(
  ...
  for (let idx = 0; idx < maxLen; idx++) {
    ...
    // divergence — only the textual content matters (mirrors rewind.diff).
    const diverged =
