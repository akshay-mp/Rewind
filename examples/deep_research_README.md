# Deep Research under TimeTravel
...
> | **[`deep_research_demo.py`](./deep_research_demo.py)** | Headless / CI / scripting — same 8-step agent, runs the full capture→frozen→branch loop in pure Python via TimeTravel's real engine. | ✅ Working |
...
(LangGraph's multi-node deep-research agent) running under TimeTravel's time-travel
debugger. This proves TimeTravel works on a real, multi-node, tool-calling agent —
...
```bash
OPENAI_BASE_URL=http://localhost:11434/v1 TIMETRAVEL_MODEL=qwen3:32b \
  python examples/deep_research.py
```
...
`openai...Completions.create`. TimeTravel intercepts **there**:
...
- **Python version caveat (3.14):** `open-deep-research` transitively pulls
  ...
  On Python 3.11–3.13 a plain `pip install agent-timetravel[deepresearch]` works.
...
- The agent runs with `search_api="none"` (no external web search) so the
  ...
  `@timetravel.tool(...)` so its results are cached during frozen replay.
