...
import type { BreakpointRule, OutputAssertions, PausedStep, PricingProfile, PromptVersion } from "@/lib/rewind/types";
import { continueFromSavedState, continueUntilNextLlm, patchRunControl, persistPromptVersion, persistPromptVersionResult, postDecision, restartSessionFrom, rerunEditedStep, stopSessionForInspection } from "@/lib/rewind/session-client";
...
export function StepPanel({ sessionId, step, stepNumber, pricing, canRewind, canStepForward, readOnly = false, onReturnToCurrent }: StepPanelProps) {
  ...
  const handleRunUntilBreakpoint = async () => {
    ...
    try {
      ...
      const breakpoints = useRewindStore.getState().breakpoints;
  ...
  const armPauseAfterCurrent = async () => {
    const breakpoints: BreakpointRule[] = useRewindStore.getState().breakpoints;
  ...
  const handleApplyEdit = async () => {
    ...
    if (isCompleted) {
      setBusy(true);
      try {
        const runnerRef = useRewindStore.getState().liveSession?.runnerRef ?? "agent";
        const traceId = useRewindStore.getState().liveSession?.traceId ?? "";
        const now = Date.now();
        const baseline: PromptVersion = {
          ...
          branchId: useRewindStore.getState().liveSession?.branchId,
          pricing,
        ...
        useRewindStore.getState().addPromptVersion(baseline);
        ...
        useRewindStore.getState().addPromptVersion(variant);
  ...
  const handleRewind = async () => {
    ...
    try {
      ...
      if (!useRewindStore.getState().restorePreviousStep()) {
  ...
  const handleStepForward = () => {
    if (!useRewindStore.getState().restoreNextStep()) {
  ...
  const runAssertions = () => {
    ...
    useRewindStore.getState().setStepAssertions(assertions, result);
    const traceId = useRewindStore.getState().liveSession?.traceId;
    if (traceId) {
    ...
    const versions = useRewindStore.getState().liveSession?.promptVersions
      .filter((version) => version.cursor === step.cursor) ?? [];
    for (const version of versions) {
      const updated = { ...version, assertions, assertionResult: result };
      useRewindStore.getState().updatePromptVersion(version.id, { assertions, assertionResult: result });
      if (traceId) {
  ...
  const saveReview = (verdict: "accepted" | "rejected") => {
    useRewindStore.getState().setStepReview(reviewNote, verdict);
    const traceId = useRewindStore.getState().liveSession?.traceId;
    const versions = useRewindStore.getState().liveSession?.promptVersions
      .filter((version) => version.cursor === step.cursor) ?? [];
    for (const version of versions) {
      const updated = { ...version, reviewNote, reviewVerdict: verdict };
      useRewindStore.getState().updatePromptVersion(version.id, { reviewNote, reviewVerdict: verdict });
  ...
  const runCustomEvaluator = async () => {
    ...
    try {
      ...
      for (const version of versions) {
        ...
        useRewindStore.getState().updatePromptVersion(version.id, { evaluatorNames, evaluatorResults });
        if (traceId) {
          void persistPromptVersion(traceId, updated);
          if (updated.status === "completed") void persistPromptVersionResult(updated.id, updated);
        }
      ...
      <div className="flex flex-wrap items-center gap-2 border-t border-white/10 bg-black/10 px-5 py-4">
        {readOnly ? (
          ...
          <>
            ...
            {canStepForward && (
              <Button
                ...
                disabled={busy}
                className={canRewind ? "bg-emerald-500 text-emerald-950 hover:bg-emerald-400" : "ml-auto bg-emerald-500 text-emerald-950 hover:bg-emerald-400"}
              >
          ...
          <>
            ...
            >
              <CornerUpLeft className="mr-1.5 size-4" /> Step Back / Rewind
            </Button>
