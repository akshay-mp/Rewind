...
export interface StepUsage {
  ...
  final_tokens: number;
  total_tokens: number;
  /** True when Rewind calculated a local approximation. */
  estimated: boolean;
}
...
export interface PausedStep {
  ...
  /** A prior captured step restored locally by the inspection rewind control. */
  restored?: boolean;
  reviewNote?: string;
...
export interface LiveSession {
  ...
  /** Captured steps available for local-only forward navigation after rewind. */
